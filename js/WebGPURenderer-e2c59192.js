...
import {
  ...
  S as Pe,
  a as Te,
} from './SharedSystems-b5f35142.js'
import { C as w } from './CanvasPool-3a305dd3.js'
const y = L.for2d()
class O {
  init() {
    const e = T({ name: 'batch', bits: [D, z(R), C] })
    this._shader = new v({ gpuProgram: e, groups: {} })
  }
  start(e, t) {
    const r = e.renderer,
      s = r.encoder,
  ...
  execute(e, t) {
    ...
    const n = s.bindGroup.getBindGroup(t.bindGroup, r, 1),
      o = s.pipeline.getPipeline(this._geometry, r, y)
...
O.extension = { type: [l.WebGPUPipesAdaptor], name: 'batch' }
class H {
  constructor(e) {
...
H.extension = { type: [l.WebGPUSystem], name: 'bindGroup' }
class F {
  constructor() {
  ...
  createGPUBuffer(e) {
    ...
    return (
      (e._updateID = 0),
      e.data && (ie(e.data.buffer, t.getMappedRange()), t.unmap()),
...
F.extension = { type: [l.WebGPUSystem], name: 'buffer' }
class Ce {
...
I.extension = { type: [l.WebGPUSystem], name: 'colorMask' }
class U {
  constructor(e) {
    this._renderer = e
  }
...
W.extension = { type: [l.WebGPUSystem], name: 'encoder', priority: 1 }
class V {
  constructor(e) {
...
V.extension = { type: [l.WebGPUSystem], name: 'stencil' }
const x = {
  i32: { align: 4, size: 4 },
  u32: { align: 4, size: 4 },
  f32: { align: 4, size: 4 },
...
function Ue(a, e) {
  const { size: t, align: r } = x[a.data.type],
    s = (r - t) / 4
  return `
         v = uv.${a.data.name};
         ${e !== 0 ? `offset += ${e};` : ''}
...
function Me(a) {
  return me(a, 'uboWgsl', Ue, _e)
}
class N extends be {
  constructor() {
    super({ createUboElements: ve, generateUboSync: Me })
  }
}
N.extension = { type: [l.WebGPUSystem], name: 'ubo' }
const _ = 128
class j {
  constructor(e) {
    ;(this._bindGroupHash = Object.create(null)),
      ...
      (this._renderer = e),
      (this._batchBuffer = new Ce({ minUniformOffsetAlignment: _ }))
    const t = 256 / _
    for (let r = 0; r < t; r++) {
      let s = S.UNIFORM | S.COPY_DST
      r === 0 && (s |= S.COPY_SRC),
        this._buffers.push(new ne({ data: this._batchBuffer.data, usage: s }))
    }
  }
  renderEnd() {
  ...
  getUniformBindGroup(e, t) {
    ...
    return (
      ...
      (this._bindGroupHash[e.uid] = this._getBindGroup(s / _)),
...
j.extension = { type: [l.WebGPUPipes], name: 'uniformBatch' }
const we = {
  'point-list': 0,
  'line-list': 1,
  'line-strip': 2,
...
class K {
  ...
  getPipeline(e, t, r, s) {
    e._layoutKey || (Ge(e, t.attributeData), this._generateBufferKey(e)),
      (s = s || e.topology)
    const i = Re(e._layoutKey, t._layoutKey, r.data, r._blendModeId, we[s])
  ...
  _createPipeline(e, t, r, s) {
    ...
    o[0].writeMask =
      this._stencilMode === P.RENDERING_MASK_ADD ? 0 : this._colorMask
    ...
    return (
      e.buffers.forEach((r) => {
        ...
        for (const n in e.attributes) {
          ...
          o.buffer === r &&
            ((s.arrayStride = o.stride),
            (s.stepMode = o.instance ? 'instance' : 'vertex'),
            i.push({
...
K.extension = { type: [l.WebGPUSystem], name: 'pipeline' }
class Ae {
  constructor() {
...
class ke {
  ...
  getDescriptor(e, t, r) {
    ...
    const s = this._renderTargetSystem,
      ...
      n = e.colorTextures.map((c, f) => {
        ...
        h
          ...
          i.msaaTextures[f] &&
            ((d = p),
            (p = this._renderer.texture.getTextureView(i.msaaTextures[f])))
    ...
    if (
      ...
      const c = t & b.STENCIL ? 'clear' : 'load',
        f = t & b.DEPTH ? 'clear' : 'load'
      o = {
  ...
  clear(e, t = !0, r, s) {
    ...
    if (n.commandEncoder === null) {
      const c = o.createCommandEncoder(),
        f = this.getDescriptor(e, t, r),
        h = c.beginRenderPass(f)
...
Y.extension = { type: [l.WebGPUSystem], name: 'shader' }
const m = {}
m.normal = {
  alpha: {
    srcFactor: 'one',
...
class $ {
  ...
  getColorTargets(e) {
    return [
      { format: 'bgra8unorm', writeMask: 0, blend: m[e.blendMode] || m.normal },
    ]
  }
  destroy() {
    this.gpu = null
...
$.extension = { type: [l.WebGPUSystem], name: 'state' }
const Le = {
    type: 'image',
    upload(a, e, t) {
      const r = a.resource,
  ...
  De = { blockBytes: 4, blockWidth: 1, blockHeight: 1 },
  ze = {
    type: 'compressed',
    upload(a, e, t) {
      let r = a.pixelWidth,
        s = a.pixelHeight
      const i = X[a.format] || De
...
class Q {
  constructor(e) {
    ;(this.managedTextures = []),
      ...
      (this._uploads = { image: J, buffer: Le, video: Oe, compressed: ze }),
      (this._renderer = e)
  }
  contextChange(e) {
    this._gpu = e
  ...
  initSource(e) {
    ...
    const r = X[e.format] || { blockBytes: 4, blockWidth: 1, blockHeight: 1 },
  ...
  _createTextureBindGroup(e) {
    const t = e.source,
      r = t.uid
    return (
      (this._bindGroupHash[r] = new A({ 0: t, 1: t.style })),
  ...
  getPixels(e) {
    const t = this.generateCanvas(e),
      r = w.getOptimalCanvasAndContext(t.width, t.height),
      s = r.context
    s.drawImage(t, 0, 0)
...
Q.extension = { type: [l.WebGPUSystem], name: 'texture' }
class Z {
  init() {
    const e = new pe({
      ...
      t = T({ name: 'graphics', bits: [D, z(R), le, C] })
    this.shader = new v({ gpuProgram: t, resources: { localUniforms: e } })
  }
  execute(e, t) {
    const r = t.context,
      ...
      i = e.renderer,
      n = i.graphicsContext,
      { geometry: o, instructions: u } = n.getContextRenderData(r),
      c = i.encoder
    c.setPipelineFromGeometryProgramAndState(o, s.gpuProgram, e.state),
      c.setGeometry(o)
    ...
    for (let d = 0; d < u.instructionSize; d++) {
      ...
      if (((s.groups[1] = g.bindGroup), !g.gpuBindGroup)) {
        const B = g.textures
        ;(g.bindGroup = E(B.textures, B.count)),
          (g.gpuBindGroup = i.bindGroup.getBindGroup(
            g.bindGroup,
            s.gpuProgram,
            1,
          ))
      }
      c.setBindGroup(1, g.bindGroup, s.gpuProgram),
        c.renderPassEncoder.drawIndexed(g.size, 1, g.start)
    }
  }
  destroy() {
...
Z.extension = { type: [l.WebGPUPipesAdaptor], name: 'graphics' }
class ee {
  init() {
    const e = T({ name: 'mesh', bits: [fe, Se, C] })
    this._shader = new v({
      gpuProgram: e,
      resources: {
...
ee.extension = { type: [l.WebGPUPipesAdaptor], name: 'mesh' }
const Fe = [...Pe, N, W, U, F, Q, q, Y, $, K, I, V, H],
  Ie = [...Te, j],
  We = [O, ee, Z],
  te = [],
  re = [],
  se = []
G.handleByNamedList(l.WebGPUSystem, te)
G.handleByNamedList(l.WebGPUPipes, re)
G.handleByNamedList(l.WebGPUPipesAdaptor, se)
G.add(...Fe, ...Ie, ...We)
class Ye extends de {
  constructor() {
    const e = {
      ...
      systems: te,
      renderPipes: re,
      renderPipeAdaptors: se,
    }
    super(e)
  }
}
export { Ye as WebGPURenderer }
